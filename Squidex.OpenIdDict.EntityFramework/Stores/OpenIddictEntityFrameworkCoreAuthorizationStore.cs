...
/// <typeparam name="TContext">The type of the Entity Framework database context.</typeparam>
/// <typeparam name="TKey">The type of the entity primary keys.</typeparam>
public class OpenIddictEntityFrameworkCoreAuthorizationStore<TAuthorization, TToken, TContext, TKey> : IOpenIddictAuthorizationStore<TAuthorization>
    ...
    where TKey : notnull, IEquatable<TKey>
{
        ...
        string subject, string client, CancellationToken cancellationToken)
    {
        ...
        return (from authorization in Authorizations.AsTracking()
        ...
        ImmutableArray<string> scopes, CancellationToken cancellationToken)
    {
        ...
        {
            var authorizations = (from authorization in Authorizations.AsTracking()
                                  where authorization.Subject == subject &&
                                        authorization.Status == status &&
                                        authorization.Type == type &&
                                        authorization.ApplicationId == client
        ...
        string identifier, CancellationToken cancellationToken)
    {
        ...
        return (from authorization in Authorizations.AsTracking()
                where authorization.ApplicationId.Equals(identifier)
                select authorization).AsAsyncEnumerable(cancellationToken);
    }
